...
class PathHandler:
    ...
    def ensure_wsl_path(path):
        ...
        return PathHandler.windows_to_wsl(path)
    ...
    def join_wsl_paths(*paths):
        ...
        return PathHandler.windows_to_wsl(joined)
    
    @staticmethod
    def to_wsl(windows_path):
        """Convert a Windows path to WSL format (alias for windows_to_wsl)."""
        return PathHandler.windows_to_wsl(windows_path)
    ...
    def wrap_command(command, input_file=None, output_file=None):
        ...
        if input_file:
            wsl_input = PathHandler.to_wsl(input_file)
            wrapped_cmd = f"{wrapped_cmd} < {wsl_input}"
        
        if output_file:
            wsl_output = PathHandler.to_wsl(output_file)
